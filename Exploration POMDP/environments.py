...
class GridworldEnv(gym.Env):
    ...
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0):
        self.grid_size = grid_size
        self.observation_space = spaces.Discrete(self.grid_size**2)   # The states in the MDP representation, called observation because of gym namings
...
class GridworldPOMDPEnvGoalless(GridworldEnvGoalless):
    ...
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, steepness=15):
        # Initialize the underlying Gridworld MDP
        super().__init__(grid_size=grid_size, time_horizon=time_horizon, prob=prob, randomize=randomize)
        # Initialize all the POMDP specific variables
        self.observation_space = spaces.Discrete(self.grid_size**2)
        self.steepness = steepness
        self.observation_matrix = self.build_observation_matrix()
...
class GridworldPOMDPEnvBiModal(GridworldEnvGoalless):
    ...
    '''
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, steepness=15, shift_amount=0.80):
        # Initialize the underlying Gridworld MDP
        super().__init__(grid_size=grid_size, time_horizon=time_horizon, randomize=randomize, prob=prob)
        # Initialize all the POMDP specific variables
...
class GridworldPOMDPEnvAsymm(GridworldEnvGoalless):
    ...
    Args:
     ...
     - shift_amount: a number that represents how much to shift the gaussian from the mean.
    '''
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, steepness=15, shift_amount=0.1):
        # Initialize the underlying Gridworld MDP
        super().__init__(grid_size=grid_size, time_horizon=time_horizon, randomize=randomize, prob=prob)
        # Initialize all the POMDP specific variables
        self.shift_amount = shift_amount
...
class CorridorGridworldEnv(gym.Env):
    metadata = {'render.modes': ['human']}
...
class CorridorGridworldEnvGoalless(CorridorGridworldEnv):
    '''
    This extension of the environment work functionally like the previous, but it removes the ending goal state.
    '''
    def step(self, action):
...
class CorridorGridworldPOMDPEnv(CorridorGridworldEnv):
    '''
    This class implements the extension of the upper Gridworld MDP, making it become a POMDP by inserting the observation mechanism.
    The observations here are modeled as a probability of returning the real state of the MDP, in a Gaussian fashion based on the distance of the observed state from the real current state.
    ...
    Args:
     ...
     - steepness: a parameter to control the steepness of the Gaussian distribution that models the observation probability from the real state. A higher value makes it steeper.
    '''
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, length_corridor=3, steepness=15):
        # Initialize the underlying Gridworld MDP
...
class CorridorGridworldPOMDPEnvGoalless(CorridorGridworldEnvGoalless):
    '''
    This extension of the environment work functionally like the previous, but it removes the ending goal state.
    In the POMDP case it also adds the observation mechanism.
    '''
...
class CorridorGridworldPOMDPEnvBiModal(CorridorGridworldEnvGoalless):
    '''
    This class implements the extension of the upper Gridworld MDP, making it become a POMDP by inserting the observation mechanism.
    The observations here are modeled as a probability of returning the real state of the MDP, in a Gaussian fashion based on the distance of the observed state from the real current state.
    Moreover in this instance the observatino will be shifted in order to be bimodal with respect to the true state.
    ...
    Args:
     ...
     - shift_amount: a number that represents how much to shift the gaussian from the mean.
    '''
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, length_corridor=3, steepness=15, shift_amount=0.80):
        # Initialize the underlying Gridworld MDP
        super().__init__(grid_size=grid_size, time_horizon=time_horizon, randomize=randomize, prob=prob)
        # Initialize all the POMDP specific variables
        self.shift_amount = shift_amount
...
class CorridorGridworldPOMDPEnvAsymm(CorridorGridworldEnvGoalless):
    '''
    This class implements the extension of the upper Gridworld MDP, making it become a POMDP by inserting the observation mechanism.
    The observations here are modeled as a probability of returning the real state of the MDP, in a Gaussian fashion based on the distance of the observed state from the real current state.
    Moreover in this instance the observatino will be shifted in order to be asymmetric with respect to the true state.
    ...
    Args:
     ...
     - shift_amount: a number that represents how much to shift the gaussian from the mean.
    '''
    def __init__(self, grid_size=5, time_horizon=-1, prob=0.1, randomize=0, length_corridor=3, steepness=15, shift_amount=0.1):
        # Initialize the underlying Gridworld MDP
        super().__init__(grid_size=grid_size, time_horizon=time_horizon, randomize=randomize, prob=prob)
        # Initialize all the POMDP specific variables
        self.shift_amount = shift_amount
